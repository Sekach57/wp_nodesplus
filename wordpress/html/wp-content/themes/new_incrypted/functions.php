...
function incrypted_scripts() {
    ...
    $is_nodes_endpoint = false;
    if ( function_exists( 'is_wc_endpoint_url' ) && is_wc_endpoint_url( 'nodes' ) ) {
        $is_nodes_endpoint = true;
    } else {
        $request_path = trim( parse_url( $_SERVER['REQUEST_URI'] ?? '', PHP_URL_PATH ) ?: '', '/' );
        if ( $request_path !== '' && in_array( 'nodes', explode( '/', $request_path ), true ) ) {
            $is_nodes_endpoint = true;
        }
    }
    $has_nodes_block = false;
    if ( function_exists( 'has_shortcode' ) ) {
        $post_content = '';
        if ( is_singular() ) {
            $post_content = get_post_field( 'post_content', get_the_ID() );
        }
        if ( $post_content !== '' && has_shortcode( $post_content, 'custom_nodes_list' ) ) {
            $has_nodes_block = true;
        }
    }
    if ( $is_nodes_endpoint && function_exists( 'is_account_page' ) && is_account_page() ) {
        wp_enqueue_style( 'incrypted-nodes-cards', get_template_directory_uri() . '/css/nodes-cards.css', array(), _S_VERSION );
    } elseif ( $has_nodes_block || is_front_page() ) {
        wp_enqueue_style( 'incrypted-nodes-cards', get_template_directory_uri() . '/css/nodes-cards.css', array(), _S_VERSION );
    }
